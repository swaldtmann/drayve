...
Drayve manages the **frame** — infrastructure that every server needs. Services are independent and connect via Docker labels.

| | Frame (Ansible) | Services (admin-managed) |
|---|---|---|
| **Managed by** | `make provision` / `make deploy` | Admin creates `compose.yml` + `.env` |
| **Lives in** | `/opt/drayve/frame/` on server | `/opt/drayve/services/<name>/` on server |
| **Idempotent** | Yes (Ansible) | N/A (admin responsibility) |
| **Interface** | `stack.yaml` | Docker labels on `drayve` network |

### Docker labels as contract

Services declare their integration with the frame via labels:

| Label namespace | What happens |
|---|---|
| `traefik.*` | Routing, TLS, middleware — standard Traefik |
| `drayve.landing.*` | Landing page: name, icon, category. No label → not on the page |
| `drayve.monitoring.*` | Custom Grafana dashboard from `dashboards/<name>.json`. No label → generic container dashboard only |

All scanning happens at **deploy time** (from compose files), not at runtime (from containers). A service that is restarting does not disappear from the landing page.

## Directory layout on the server

```
/opt/drayve/
|-- frame/                      Ansible-managed infrastructure
|   |-- docker-compose.yml
|   |-- .env
|   |-- traefik/
|   |   |-- config/
...
|   +-- landing/
|-- services/                   Admin-managed services
|   |-- nextcloud/
|   |   |-- compose.yml
|   |   +-- .env
|   +-- xwiki/
|       |-- compose.yml
|       +-- .env
|-- dashboards/                 Custom Grafana dashboards (drop-in)
|   +-- nextcloud.json
|-- config/
...
| `authentik` | Authentik (planned) | Enterprise SSO with AD/SAML/OIDC |
